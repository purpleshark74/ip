...
# Present Changes Visually for Bobby

This project is the Bobby Java project and uses JDK 25 for Java builds. The
visual-diff generator itself is Python standard-library code, so do not change
the project's Java configuration or install Java dependencies when using this
skill.
...
4. Run the bundled generator from the repository root:
   ...
   On systems where `python` is not available, use the locally installed
   Python 3 executable instead. The generator requires no third-party package.
